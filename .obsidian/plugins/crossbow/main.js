...
// src/view/tree/treeNode.ts
var import_obsidian = require("obsidian");
var TreeNode = class extends HTMLElement {
  constructor(value, manager) {
    ...
    const isLeaf = this.value.children === void 0;
  ...
  get childTreeNodes() {
    return this.childrenWrapper ? Array.from(this.childrenWrapper.children) : [];
  }
  static register() {
  ...
  generateChildren() {
    if (!this.childrenWrapper || !this.value.children)
      return;
    if (this.childrenWrapper.children.length > 0)
      return;
    const nodes = this.value.children.map((child) => new TreeNode(child, this.manager));
    this.childrenWrapper.replaceChildren(...nodes);
  }
  isCollapsed() {
    if (!this.childrenWrapper || !this.iconWrapper)
      return true;
...
// src/view/tree/treeUpdater.ts
var equals = (a, b) => a.uid === b.uid;
var TreeUpdater = class {
  update(newNodes, oldNodes) {
    const updates = [];
    for (let i = 0; i < newNodes.length; i++) {
      const newNode = newNodes[i];
      const index = oldNodes.findIndex((oldNode) => equals(oldNode.value, newNode.value));
      const existingNode = index !== -1 ? oldNodes.splice(index, 1)[0] : void 0;
      if (existingNode) {
        existingNode.childTreeNodes.filter((child) => !child.isCollapsed()).forEach((expandedChild) => {
          const child = newNode.childTreeNodes.find((child2) => equals(child2.value, expandedChild.value));
          if (child) {
            child.expand();
...
// src/view/tree/tree.ts
var registerTreeElements = () => {
  TreeNode.register();
  Tree.register();
};
var Tree = class extends HTMLElement {
  ...
  update(data) {
    ...
    const batch = this.treeUpdater.update(data.map((d) => new TreeNode(d, this)), oldNodes);
    requestAnimationFrame(() => {
      batch.forEach((update) => update(this));
    });
  }
  getChildTreeNodes() {
    return this.children.length > 0 ? Array.from(this.children) : [];
  }
};
...
var _CrossbowView = class extends import_obsidian2.ItemView {
  ...
  clear() {
    var _a;
    (_a = this.tree) == null ? void 0 : _a.remove();
    this.tree = null;
  }
...
var Suggestion = class {
  constructor(word, matches, matchOccurrences) {
    this.parent = null;
    this.children = [];
    this.word = word;
    this.children = matchOccurrences.map((p) => {
      const matchOccurrenceEnd = { ch: p.ch + word.length, line: p.line };
...
var Occurrence = class {
  constructor(parent, editorPosition, editorEndPosition, cacheMatches) {
    this.children = [];
    this.parent = parent;
...
var Match = class {
  ...
  get actions() {
    return [
      {
        name: "Use",
        icon: "lucide-inspect" /* Inspect */,
        callback(ev, ctx) {
          var _a;
          ((_a = ctx.self.parentElement) == null ? void 0 : _a.parentElement).setDisable();
        ...
        callback(ev, ctx) {
          ...
          leaf.openFile(this.cacheMatch.file, { active: false }).then(() => {
            ...
            if (leaf.view instanceof import_obsidian4.MarkdownView) {
              ...
              if ((_a = this.cacheMatch.item) == null ? void 0 : _a.position) {
                const from = {
  ...
  createLink() {
    ...
    return this.cacheMatch.item ? app.fileManager.generateMarkdownLink(this.cacheMatch.file, this.cacheMatch.text, "#" + this.cacheMatch.text, word) : app.fileManager.generateMarkdownLink(this.cacheMatch.file, this.cacheMatch.text, void 0, word);
  }
};
      ...
      for (let j = 0; j < cacheValues.length; j++) {
        ...
        for (let k = 0; k < cacheEntries.length; k++) {
          ...
          if (matches.length >= 300)
            continue;
          if (!settings.suggestInSameFile && cacheValue.file === currentFile)
            continue;
          if (cacheKey === word) {
            matches.push({ ...cacheValue, rank: "\u{1F3C6}" });
            continue;
          }
          if (word.length <= 3)
            continue;
          ...
          if (lowercaseCacheKey === lowercaseWord) {
            matches.push({ ...cacheValue, rank: "\u{1F947}" });
            continue;
          }
          if (1 / cacheKey.length * word.length <= 0.2) {
            matches.push({ ...cacheValue, rank: "\u{1F949}" });
            continue;
          }
          matches.push({ ...cacheValue, rank: "\u{1F948}" });
        }
      }
      if (matches.length > 0) {
...
var MARKDOWN_ASTERISK_EMPHASIS_REGEX = /([*]+)(\S)(.*?\S)??(\1)/g;
var _CrossbowTokenizationService = class {
  constructor() {
    this.SKIP_REGEX = /\s/;
  }
  getWordLookupFromEditor(targetEditor) {
    ...
    const plainText = _CrossbowTokenizationService.redactText(rawText);
    for (let i = 0; i < plainText.length; i++) {
      if (plainText[i].match(this.SKIP_REGEX))
        ...
        while (plainText[i] && !plainText[i].match(this.SKIP_REGEX))
          word += plainText[i++];
        const cleanWord = _CrossbowTokenizationService.cleanWord(word);
        if (cleanWord.length <= 0)
          continue;
        const wordStartOffset = word.indexOf(cleanWord[0]);
        if (wordStartOffset > 0)
          pos.ch += wordStartOffset;
        if (cleanWord in wordLookup)
          wordLookup[cleanWord].push(pos);
        else
          wordLookup[cleanWord] = [pos];
      }
    }
    return wordLookup;
  }
  static redactText(text) {
    return text.replace(MARKDOWN_ASTERISK_EMPHASIS_REGEX, (m, g1, g2, g3) => this.muteWord(g1) + g2 + g3 + this.muteWord(g1)).replace(MARKDOWN_CODE_BLOCK_REGEX, (m) => this.muteString(m)).replace(MARKDOWN_LATEX_BLOCK_REGEX, (m) => this.muteString(m)).replace(MARKDOWN_LATEX_INLINE_REGEX, (m) => this.muteString(m)).replace(MARKDOWN_LINKS_AND_IMAGES_REGEX, (m) => this.muteString(m)).replace(OBSIDIAN_METADATA_REGEX, (m) => this.muteString(m)).replace(OBSIDIAN_TAG_REGEX, (m) => this.muteString(m)).replace(OBSIDIAN_LINKS_REGEX, (m) => this.muteString(m)).replace(HTML_COMMENT_REGEX, (m) => this.muteString(m)).replace(HTML_TAG_REGEX, (m) => this.muteString(m));
  }
  static cleanWord(word) {
    return word.replace(/[^a-z0-9äöü'-]/gi, "").replace(/^[’'-]+|[’'-]+$/gi, "");
  }
};
var CrossbowTokenizationService = _CrossbowTokenizationService;
CrossbowTokenizationService.muteString = (str) => str.replace(/[^\r\n]+/g, (m) => " ".repeat(m.length));
CrossbowTokenizationService.muteWord = (word) => " ".repeat(word.length);
...
var CrossbowPlugin = class extends import_obsidian6.Plugin {
  constructor(app2, manifest) {
    ...
    this.onFileOpen = () => {
      ...
      this.setActiveFile();
      this.loggingService.debugLog(`\u26A1File opened. '${(_a = this.currentFile) == null ? void 0 : _a.name}'`);
  ...
  runWithoutCacheUpdate(fileHasChanged) {
    var _a;
    const fileView = (_a = app.workspace.getLeavesOfType("markdown").find((leaf) => leaf.view instanceof import_obsidian6.MarkdownView && leaf.view.file === this.currentFile)) == null ? void 0 : _a.view;
    if (!fileView)
      return;
  ...
  setActiveFile() {
    ...
    if ((leaf == null ? void 0 : leaf.view) instanceof import_obsidian6.MarkdownView && leaf.view.file) {
